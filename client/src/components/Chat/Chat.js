...
const Chat = ({ display, roomId, firstName }) => {
  ...
  useEffect(() => {
    socket.on("FE-receive-message", ({ msg, sender, firstName }) => {
      setMsg((msgs) => [...msgs, { sender, msg, firstName }]);
    });
  }, []);
  ...
  return (
    ...
    >
      ...
      <ChatArea>
        <MessageList>
          {msg &&
            msg.map(({ sender, msg, firstName }, idx) => {
